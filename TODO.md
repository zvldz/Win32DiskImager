...
- **`--check-updates` for CLI** — explicit one-shot version check, mirrors the GUI's checker but never auto-runs. CLI defaults stay silent so scripted invocations don't hit network on every run / hammer the GitHub anonymous rate limit. Implementation reuses the same release-API URL the GUI uses; on a new tag it prints `Latest: vX.Y.Z. Current: vA.B.C. Download: <github release page url>`. Not implemented yet — added when needed.
