...
- Starting a write within a second or two of inserting a card no longer fails with "Could not open the target device for writing". The card appears in the Device list as soon as Windows reports media present, which is before its controller has finished initialising — and a reader whose USB port is waking from power saving looks the same. Opening the disk in that window returned "device not found", which was treated as fatal and failed instantly; it is now retried for a few seconds. When the device really is gone, the message says so instead of blaming other applications. Both GUI and CLI.
